...
# ---------------------------------------------------------------------------
def build_hybrid_database(mms_filepath, db_path, output_filepath):
    """
    Scrive l'output in formato JSON Lines (un oggetto JSON per riga) invece
    che un unico grande array JSON. Per una pipeline RAG a valle è quasi
    sempre la scelta giusta: puoi rileggere e fare chunking del dataset
    riga per riga, senza dover ri-parsare un array enorme con ijson.
    Se ti serve comunque un unico array JSON valido, vedi la nota in fondo.
    """
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def main():
    ...
    parser.add_argument("--output", default="mms_ibrido_vettoriale.jsonl")
    ...
    log.info("2/2 - Fusione MMS + Foundation con output in streaming...")
    build_hybrid_database(args.mms, args.index_db, args.output)
