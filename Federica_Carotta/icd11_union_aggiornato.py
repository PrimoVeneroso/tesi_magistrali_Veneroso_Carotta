...
def create_lookup_link(lookup_title):  
    ...
    for link, titolo in lookup_title.items():
        if titolo:
            ...
            titolo_norm = str(titolo).strip().lower() # normalizziamo il titolo (tutto minuscolo e senza spazi extra)
            
            if titolo_norm not in lookup_link:
                lookup_link[titolo_norm] = {
                    "foundationReference": "",
                    "linearizationReference": ""
                }
            
            # assegniamo il link al posto giusto in base al tipo di URL
            if "/entity/" in str(link):
                lookup_link[titolo_norm]["foundationReference"] = link
            else:
                lookup_link[titolo_norm]["linearizationReference"] = link
                        ...
                        if isinstance(element_list,str): # se è una stringa, allora devo fare due cose (anche se so già che quasi sempre un link, ma meglio verificarlo)
                            ...
                            if not foundation_ref and not linearization_ref and title:
                                ...
                                if title_norm in lookup_link:
                                    ...
                                    # Se ci sono, li assegniamo entrambi ai rispettivi campi!
                                    if refs_found["foundationReference"]:
                                        foundation_ref = refs_found["foundationReference"]
                                    if refs_found["linearizationReference"]:
                                        linearization_ref = refs_found["linearizationReference"]
...
def count_entity_with_reference(dataset, campo):
    titoli_unici = set()
    for entity in dataset:
        for element in entity.get(campo, []):
            if isinstance(element, dict):
                ...
                # Se c'è la foundation_ref OPPURE la linearization_ref, allora ha un link!
                if foundation_ref or linearization_ref:
                    titolo = element.get("title", "")
                    if titolo: # Ci assicuriamo che il titolo esista
...
titoli_con_reference_totali = set()
...
all_final_fields = [
    ...
    "synonyms_and_inclusions", 
    "exclusion_mms_foundation"
]

for campo in all_final_fields:
    entity_without_reference = count_entity_without_reference(data_mmms_foundation, campo)
    print(f"Entità senza reference: - {campo}: {len(entity_without_reference)}")
    titoli_senza_reference_totali.update(entity_without_reference)

    entity_with_reference = count_entity_with_reference(data_mmms_foundation, campo)
    titoli_con_reference_totali.update(entity_with_reference)
print(f"Numero TOTALE di titoli unici CON reference: {len(titoli_con_reference_totali)}")
...
file_con_ref = "titoli_con_reference.txt"
with open(file_con_ref, "w", encoding="utf-8") as f:
    for titolo in sorted(titoli_con_reference_totali):
        f.write(f"{titolo}\n")    
